...
document.querySelector('#sil').addEventListener('click', function () {
    if (selectedRow) {
        ...
        if (stokKodu) {
            // Modal'ı aç
            var silmeModal = document.getElementById('silmeModal');
            var instance = M.Modal.getInstance(silmeModal);
            instance.open();

            // "Evet" butonuna tıklanınca silme işlemi
            document.querySelector('#confirmDelete').addEventListener('click', function () {
                window.location.href = '/Admin/StokKartıSil?stokKodu=' + stokKodu;
            });
        }
    } else {
...
document.querySelector('#duzenle').addEventListener('click', function () {
    if (selectedRow) {
        let stokKodu = selectedRow.data()[0];  // İlk sütundaki veriyi al, stokKodu olarak varsayalım
        if (stokKodu) {
            window.location.href = '/Admin/StokKartiDuzenle?stokKodu=' + stokKodu;
        }
    } else {
