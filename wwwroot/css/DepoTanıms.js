...
document.querySelector('#sil').addEventListener('click', function () {
    if (selectedRow) {
        let depoKodu = selectedRow.data()[0];  // İlk sütundaki veriyi al, stokKodu olarak varsayalım
        if (depoKodu) {
            window.location.href = '/Admin/depoKartıSil?depoKodu=' + depoKodu;
        }
    } else {
...
document.querySelector('#duzenle').addEventListener('click', function () {
    if (selectedRow) {
        let depoKodu = selectedRow.data()[0];  // İlk sütundaki veriyi al, stokKodu olarak varsayalım
        if (depoKodu) {
            window.location.href = '/Admin/DepoKartiDuzenle?depoKodu=' + depoKodu;
        }
    } else {
